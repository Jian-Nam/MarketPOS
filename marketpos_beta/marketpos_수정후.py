...
class SecondWindow(QMainWindow, SecondWindow_formclass): #바코드 입력창
    ...
    def enterInfo(self):
        ...
        if self.cart.find_item(barcode) == None:
            ...
            if len(self.cart.shoppingCart) > 4:                   #shoppingCart_dict에 5개이상 담겼을 경우 더이상 실행되지 않음
                self.barcodeInput.clear()
            elif type(self.cart.find_item(barcode)) == list:
                thirdWindow = ThirdWindow(self.cart.find_item(barcode), self)
                thirdWindow.exec_()
                Chosed_product = thirdWindow.chosed_product 
                if Chosed_product ==None:
                    pass
                else:
                    self.cart.add_item(Chosed_product)
                    self.show_cart()
            else:
...
class ThirdWindow(QDialog, ThirdWindow_formclass):
    ...
    def __init__(self, lst, parent=None):
        ...
        self.chosed_product = None

    def send_info(self):
        self.chosed_product = self.lst[self.overlap_list.currentRow()]
        self.close()
