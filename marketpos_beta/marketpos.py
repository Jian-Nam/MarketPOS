...
from PyQt5.QtWidgets import QApplication, QWidget, QListWidget, QListWidgetItem, QLabel, QMainWindow, QTableWidgetItem
...
def create_pdict():  
    ...
    for k in sheet2[sh2_name]:
        if k.value:                   #None까지 딕셔너리에 key로 들어가지 않게 하기 위함
            row = str(k.row)
            product_dict[str(sheet2[sh2_barcode + row].value)] = Product([
                sheet2[sh2_name + row].value, 
                sheet2[sh2_supplier + row].value, 
                sheet2[sh2_poommok + row].value, 
                sheet2[sh2_quantity + row].value, 
                sheet2[sh2_barcode + row].value
                ])
...
class Product:	
    ...
    def __init__(self, lst):
        self.name = lst[0]
        self.supplier = lst[1]
        self.poommok = float(lst[2])
        self.quantity =	int(lst[3])
        self.multiply = math.ceil(self.poommok*self.quantity)
        self.barcode = str(lst[4])
    ...
    def enterInfo(self):
        ...
        try:
            if len(self.cart.shoppingCart) >4:                   #shoppingCart_dict에 5개이상 담겼을 경우 더이상 실행되지 않음
                ...
                self.cart.add_item(product_dict[barcode])
                self.show_cart()
